...
| R3  | Capture structural metadata from exchanges without accessing sensitive payloads.                          | **Partial**     | TSG observers receive workflow metadata; the JSON-LD validator returns successfully validated metadata field identifiers. | Sanitized semantic events and aggregate field-presence summaries.                                                                                             | `POST /api/ingest/events`; `/api/events`, `/api/report`, `/api/field-usage`.                                      | Local sanitization, central allowlists, bounded scalar validation, and no raw-payload field in the accepted event model. | E02-E05, E09, E13. Validator-driven metadata observations work, but the PoC validates configured dataset metadata rather than transferred business payloads or every possible connector representation. |
...
| R5  | Identify governed fields that were not observed in a defined evidence window.                             | **Partial**     | HTTP Data Plane JSON-LD validation result and local field-usage buckets.                                                  | `semantic-field.usage.summary` with governed standard, version, validated field ID, observation count, present count, and window.                             | `GET /api/field-usage`; Governed Semantic Field Usage dashboard card.                                             | Local minimum observations, central minimum participants, counts only, and no values.                                    | E06-E10, E13. Native two-participant summaries are implemented; local buckets remain in memory and therefore do not preserve an evidence window across restart.                                         |
...
| R14 | Optionally demonstrate the architecture with a proof-of-concept technical artifact.                       | **Implemented** | TSG demo participants export to the central service.                                                                      | Native negotiation, transfer, access, validation, and governed-field events; controlled scenarios remain available for deliberate failure cases.              | Deployed Cloud Run dashboard and executable demo blueprint.                                                       | Demo rules prohibit payload inspection and secret disclosure.                                                            | E10, E12, E13. Native mode verifies two participants and zero synthetic event publication; controlled scenarios must still be identified as synthetic evidence.                                         |
...
| HTTP Data Plane | Dataset configuration and item observation                               | `dataset.configuration.observed`                        | Adoption            | Configured artefacts, versions, and completeness.                    |
...
| HTTP Data Plane | Successful JSON-LD metadata validation and thresholded local aggregation | `semantic-field.usage.summary`                          | Adoption            | Governed-field presence derived from validator-produced identifiers. |
...
| ------------------------------------- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Business processing -> observer       | Workflow metadata, schema/configuration metadata, validation outcome.                  | Passive observer APIs; field presence comes from successfully validated identifiers and no raw payload field exists in the semantic event model. | The current PoC observes configured dataset metadata, not business-payload conformance.                                                                                                     |
...
The result establishes an observed association only. The API, dashboard, reports, and documentation must not say that a version _caused_ failures. The approved wording is:
...
| AC-04   | Dataset field observations contain validator-produced field identifiers but not their values.                                                                                       | `tno-security-gateway/libs/common-dsp/src/jsonld/validate-extra-props.test.ts` and `tno-security-gateway/apps/http-data-plane-api/src/semantic-observability/dataset-config-observer.service.test.ts`. | Implemented.                                         |
...
| AC-15   | Automatic governed-field observation consumes validator-produced field IDs, exports no values or unknown names, and produces correct denominators within the running process.       | Common DSP validator tests and HTTP Data Plane semantic-observability tests.                                                                                                                           | Implemented; restart continuity remains a known gap. |
| AC-16   | A two-participant real exchange produces central negotiation, transfer, access, validation, and field-presence events without the demo script publishing synthetic semantic events. | `run-sharing-flow.ps1 -TelemetryMode native` verifies both participant sources and all required native event types in SDO.                                                                             | Implemented and executed against the hosted PoC.     |
...
4. Persist local field-usage buckets if evidence windows must survive HTTP Data Plane restarts; extend AC-15 with a restart-continuity test.
...
- **E13 - Native architecture and event flow:** `tno-security-gateway/demo/semantic-observability-sharing/ARCHITECTURE_AND_EVENT_FLOW.md` and the native verification implemented in `scripts/run-sharing-flow.ps1`.
